...
function checkScriptUpdate()
    if not enable_autoupdate then 
        print("[RodinaMarket] Автообновление отключено")
        return 
    end
    
    print("[RodinaMarket] Начинаю проверку обновления...")
    ...
    lua_thread.create(function()
        ...
        downloadUrlToFile(json_url, e, function(id, status, p1, p2)
            -- Игнорируем промежуточные статусы загрузки, ловим только конечные
            if status == 1 then
                -- STATUS_DOWNLOADINGDATA - промежуточный статус, игнорируем
                return
            end
            ...
            if status == 2 or status == 3 or status == 4 then
                ...
                if doesFileExist(e) then
                    local k = io.open(e, 'r')
                    if k then
                        local json_text = k:read('*a')
                        k:close()
                        os.remove(e)
                        
                        if json_text and json_text ~= "" then
                            local ok, json_data = pcall(decodeJson, json_text)
                            
                            if ok and json_data and json_data.latest and json_data.updateurl then
                                local update_version = json_data.latest
                                local current_version = thisScript().version
                                
                                print(string.format("[RodinaMarket] Текущая версия: %s, на сервере: %s", current_version, update_version))
                                
                                if update_version ~= current_version then
                                    -- Обновление доступно!
                                    if DownloadState then
                                ...
                                if not ok then
                                    print("[RodinaMarket] Ошибка парсинга JSON: " .. tostring(json_data))
                                else
                                    print("[RodinaMarket] Ошибка: неверный формат JSON или пустой ответ")
                                end
                            end
                        else
                            print("[RodinaMarket] Ошибка: JSON файл пустой")
                        end
                    else
                        print("[RodinaMarket] Ошибка: не удалось открыть файл")
                    end
                else
                    print("[RodinaMarket] Ошибка: файл не был скачан")
                end
            elseif status == 5 then
                -- STATUS_ERROR
                print("[RodinaMarket] Ошибка загрузки: сетевая ошибка")
            elseif status ~= 1 then
                -- Игнорируем только промежуточные статусы, остальные - логируем
                if status > 50 or status < 1 then
                    -- Промежуточные статусы прогресса - не логируем
                    return
                end
            end
        end)
...
function main()
    ...
    -- === ПРОВЕРКА ОБНОВЛЕНИЯ СКРИПТА (ПОСЛЕ ИНИЦИАЛИЗАЦИИ) ===
    print("[RodinaMarket] Проверяю наличие обновлений...")
    if enable_autoupdate then
        checkScriptUpdate()
    else
        print("[RodinaMarket] Автообновление отключено в конфиге")
    end
