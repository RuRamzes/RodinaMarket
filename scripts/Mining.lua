...
function samp.onShowDialog(id, style, title, button1, button2, text)
    ...
    if id == D_SHELF and isAutoCollecting then
    ...
    if id == D_CONFIRM and isAutoCollecting and currentTaskType == 'collect' then
        sampSendDialogResponse(D_CONFIRM, 1, -1, ""); nextActionTime = os.clock() + 0.3; return false 
    end
end
...
imgui.OnFrame(function() return WinState[0] end, function(player)
    ...
    local rowHeight, headerHeight = 48, 35
    local contentHeight = 230 + headerHeight + (#shelvesData * rowHeight) + 40
    local maxHeight = resY * 0.85
    if contentHeight > maxHeight then contentHeight = maxHeight end
    if contentHeight < 400 then contentHeight = 400 end
    local targetW = 900
    if targetW > resX * 0.95 then targetW = resX * 0.95 end

    imgui.SetNextWindowPos(imgui.ImVec2(resX * 0.5, resY * 0.5), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
    imgui.SetNextWindowSize(imgui.ImVec2(targetW, contentHeight), imgui.Cond.Always)
    
    imgui.Begin(u8'MINING MANAGER', WinState, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoScrollbar)
    ...
    imgui.SetCursorPosX(imgui.GetWindowWidth() - closeSize - 15 - 35 - 40)
    ...
    if imgui.Button(faicons('cart_shopping'), imgui.ImVec2(closeSize + 10, closeSize)) then
        os.execute('explorer "https://t.me/rdnMarket"')
    end
    
    if imgui.IsItemHovered() then
        imgui.BeginTooltip()
        imgui.TextColored(imgui.ImVec4(1.0, 0.8, 0.0, 1.0), u8"RODINA MARKET SCRIPT")
        imgui.Separator()
        imgui.Text(u8[[
Лучший скрипт для торговли на ЦР!
- Сканирование инвентаря через CEF
- Автоматическое выставление товаров на продажу
- Автоматическое выставление товаров на скупку
- Гибкие списки с настройкой цены и количества
- Логи всех сделок, фильтры и статистика
]])
        imgui.Spacing()
        imgui.TextColored(imgui.ImVec4(0.4, 0.6, 1.0, 1.0), u8"Нажмите, чтобы перейти в Telegram канал")
        imgui.EndTooltip()
    end
    imgui.PopStyleColor(3)

    imgui.SetCursorPosX(imgui.GetWindowWidth() - closeSize - 15 - 35)
    imgui.SetCursorPosY(15)
    if imgui.Button(faicons('gear'), imgui.ImVec2(closeSize, closeSize)) then
        SettingsState[0] = not SettingsState[0]
    end

    imgui.SetCursorPosX(imgui.GetWindowWidth() - closeSize - 15)
    imgui.SetCursorPosY(15)
    if imgui.Button(faicons('xmark'), imgui.ImVec2(closeSize, closeSize)) then
    ...
    if SettingsState[0] then
        imgui.SetNextWindowPos(imgui.ImVec2(resX * 0.5, resY * 0.5), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(500, 450))
        imgui.Begin(u8"Настройки Telegram", SettingsState, imgui.WindowFlags.NoCollapse)
            imgui.Checkbox(u8"Включить Telegram уведомления", inputEnabled)
    ...
    if stats.workingCount > 0 then
        if stats.nextFullPayDays <= 0 then
            ...
            if stats.nextFullPayDays <= 2 then 
                fullColor = imgui.ImVec4(1.0, 0.6, 0.2, 1.0) 
            else 
                fullColor = imgui.ImVec4(0.2, 0.8, 0.5, 1.0) 
            end
        end
    end
    ...
    local btnHeight = 40
    local availableForCollect = 0
    local needCooling = 0
    local needStart = 0
    for _, item in ipairs(shelvesData) do
        if item.profit >= 1.0 then availableForCollect = availableForCollect + 1 end
        if item.cooling < 50 then needCooling = needCooling + 1 end
        if item.status:find("На паузе") or item.status:find("Выключ") then needStart = needStart + 1 end
    end
    ...
    if isAutoCollecting then
        ...
        if countCollect > 0 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.1, 0.7, 0.3, 0.6))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.1, 0.8, 0.3, 0.8))
        else
        ...
        if imgui.Button(faicons('coins')..u8" Собрать ("..countCollect..")", imgui.ImVec2(-1, btnHeight)) and countCollect > 0 then
            startMiningAction('collect')
        end
        imgui.PopStyleColor(2)
        imgui.NextColumn()

        if countRestore > 0 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.0, 0.5, 0.9, 0.6))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.0, 0.6, 1.0, 0.8))
        else
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(1, 1, 1, 0.05))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(1, 1, 1, 0.05))
        end
        
        if imgui.Button(faicons('droplet')..u8" Жидкость ("..countRestore..")", imgui.ImVec2(-1, btnHeight)) and countRestore > 0 then
            startMiningAction('restore')
        end
        imgui.PopStyleColor(2)
        imgui.NextColumn()

        if countStart > 0 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.9, 0.5, 0.0, 0.6))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(1.0, 0.6, 0.0, 0.8))
        else
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(1, 1, 1, 0.05))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(1, 1, 1, 0.05))
        end
        
        if imgui.Button(faicons('power_off')..u8" Запуск ("..countStart..")", imgui.ImVec2(-1, btnHeight)) and countStart > 0 then
            startMiningAction('start')
        end
        imgui.PopStyleColor(2)
        ...
        if countCollect + countRestore + countStart > 0 then
            ...
            if imgui.Button(u8"ВЫПОЛНИТЬ ВСЕ ДЕЙСТВИЯ СРАЗУ (Пробел)", imgui.ImVec2(-1, 30)) then
                 startMiningAction('all')
            end
            imgui.PopStyleColor()
        end
    ...
    local w1, w2, w3, w4, w5, w6 = cw*0.06, cw*0.14, cw*0.22, cw*0.18, cw*0.30, cw*0.10
    
    imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.12, 0.12, 0.14, 1.0))
    imgui.BeginChild("Header", imgui.ImVec2(-1, 35), false)
        imgui.Columns(6, "HeadCols", false)
        imgui.SetColumnWidth(0, w1); imgui.SetColumnWidth(1, w2); imgui.SetColumnWidth(2, w3);
        imgui.SetColumnWidth(3, w4); imgui.SetColumnWidth(4, w5); imgui.SetColumnWidth(5, w6);
        local offY = (35 - imgui.GetTextLineHeight()) / 2
        local function HeadText(txt) imgui.SetCursorPosY(offY); imgui.TextColored(imgui.ImVec4(0.5, 0.55, 0.6, 1.0), txt); imgui.NextColumn() end
        HeadText("#"); HeadText(u8"СТАТУС"); HeadText(u8"ЖИДКОСТЬ"); HeadText(u8"БАЛАНС"); HeadText(u8"ЗАПОЛНЕНИЕ"); HeadText("")
    imgui.EndChild()
    imgui.PopStyleColor()

    imgui.BeginChild("Body", imgui.ImVec2(-1, -1), true)
        imgui.Columns(6, "BodyCols", false)
        imgui.SetColumnWidth(0, w1); imgui.SetColumnWidth(1, w2); imgui.SetColumnWidth(2, w3);
        imgui.SetColumnWidth(3, w4); imgui.SetColumnWidth(4, w5); imgui.SetColumnWidth(5, w6);

        for i, item in ipairs(shelvesData) do
            local isActive = (isAutoCollecting and i == currentTaskIndex)
            local cursorYStart = imgui.GetCursorPosY()
            local centerY = (rowHeight - imgui.GetTextLineHeight()) / 2
            local centerBarY = (rowHeight - 10) / 2
            local centerBtnY = (rowHeight - 28) / 2

            imgui.SetCursorPosY(cursorYStart + centerY)
            imgui.TextColored(imgui.ImVec4(1,1,1,0.5), tostring(item.shelf))
            imgui.NextColumn()

            local stCol = item.status:find("Работает") and imgui.ImVec4(0.2, 0.8, 0.4, 1.0) or imgui.ImVec4(0.8, 0.3, 0.3, 1.0)
            imgui.SetCursorPosY(cursorYStart + centerY)
            imgui.TextColored(stCol, faicons('circle'))
            imgui.SameLine()
            imgui.Text(item.status:find("Работает") and u8"В работе" or u8"Стоп")
            imgui.NextColumn()

            imgui.SetCursorPosY(cursorYStart + centerBarY)
            local barCol = (item.cooling < 50) and imgui.ImVec4(0.9, 0.3, 0.3, 1.0) or ((item.cooling < 80) and imgui.ImVec4(1.0, 0.6, 0.0, 1.0) or imgui.ImVec4(0.0, 0.5, 0.9, 1.0))
            DrawCustomProgressBar(item.cooling / 100, imgui.ImVec2(-1, 10), barCol)
            if imgui.IsItemHovered() then imgui.SetTooltip(item.cooling.."%") end
            imgui.NextColumn()

            imgui.SetCursorPosY(cursorYStart + centerY)
            if item.profit > 0 then
                imgui.Text(string.format('%.4f', item.profit))
                imgui.SameLine()
                imgui.TextColored(imgui.ImVec4(1, 0.84, 0, 1), faicons('coin'))
            else
                imgui.TextColored(imgui.ImVec4(1,1,1,0.2), "0.0000")
            end
            imgui.NextColumn()
            ...
            if isFull then
                imgui.TextColored(imgui.ImVec4(1.0, 0.3, 0.3, 1.0), u8"ТРЕБУЕТ СБОРА")
            else
                imgui.TextColored(imgui.ImVec4(0.3, 0.7, 1.0, 1.0), u8(timeStr))
            end
            imgui.NextColumn()

            imgui.SetCursorPosY(cursorYStart + centerBtnY)
            if not isAutoCollecting then
                imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(1,1,1,0.05))
                if imgui.Button(faicons('gear').."##"..i, imgui.ImVec2(30, 28)) then
            ...
            imgui.SetCursorPosY(cursorYStart + rowHeight)
            imgui.PushStyleColor(imgui.Col.Separator, imgui.ImVec4(1,1,1,0.03))
            imgui.Separator()
            imgui.PopStyleColor()
        end
        imgui.Columns(1)
