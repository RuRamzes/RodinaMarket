...
-- ============================================================================
-- СИСТЕМА ОБНОВЛЕНИЙ (переработанная по гайду)
-- ============================================================================
...
if enable_autoupdate then
    ...
    if updater_loaded then
        ...
        if autoupdate_loaded then
            Update.json_url = UPDATE_CHECK_URL .. "?" .. tostring(os.clock())
            Update.prefix = "[RodinaMarket]: "
            Update.url = "https://github.com/RuRamzes/RodinaMarket/"
        end
    end
end

-- Простая система обновления items.json
function checkItemsUpdate()
    if App.checking_updates then return end
    App.checking_updates = true
    
    lua_thread.create(function()
        local tmp = os.tmpname()
        local version_url = ITEMS_VERSION_URL .. "?" .. tostring(os.clock())
        
        downloadUrlToFile(version_url, tmp, function(id, status)
            if status == 1 then
...
function downloadUpdatedItems()
    ...
    lua_thread.create(function()
        ...
        download_url = download_url .. "?" .. tostring(os.clock())
        
        -- Скачиваем напрямую в нужную папку
        downloadUrlToFile(download_url, ITEMS_FILE, function(id, status)
            if status == 1 then
...
-- Вспомогательные функции для работы с версиями (упрощенные)
...
local function saveJSONFile(path, data)
    ...
    if file then
        ...
        os.remove(path) 
        local success = os.rename(temp_path, path)
        if not success then
            print("[RodinaMarket] Error renaming file")
            return false
        end
        return true
    end
...
function startAvgPriceScan()
    ...
    sampAddChatMessage('[RodinaMarket] {ffff00}Откройте диалог "Ценовая статистика" (пикап на ЦР), если он не открыт.', -1)
end
...
imgui.OnFrame(function() return App.win_state[0] end, function(player)
    ...
    if imgui.Begin("Rodina Market", App.win_state, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize) then
        ...
        -- Панель обновлений items.json (если есть)
        ...
        imgui.BeginGroup()
            ...
            elseif App.active_tab == 2 then
                ...
                local filtered = getFilteredItems(Data.buyable_items, Buffers.buy_search, "buy_search_result", "last_buy_query")
...
function main()
    ...
    sampRegisterChatCommand('rupdate', function() 
        -- Проверяем обновление скрипта (если включено)
