...
function checkForUpdates()
    ...
    lua_thread.create(function()
        ...
        local update_url = UPDATE_CHECK_URL .. "?t=" .. os.time()
        downloadUrlToFile(update_url, UPDATES_FOLDER .. "\\update_temp.json", function(id, status)
            if status == 1 then
                local update_data = loadJSONFile(UPDATES_FOLDER .. "\\update_temp.json")
            ...
            -- Проверяем обновление items.json
            local items_version_url = ITEMS_VERSION_URL .. "?t=" .. os.time()
            downloadUrlToFile(items_version_url, UPDATES_FOLDER .. "\\items_version_temp.json", function(id, status)
                if status == 1 then
                    local items_version_data = loadJSONFile(UPDATES_FOLDER .. "\\items_version_temp.json")
                ...
                os.remove(UPDATES_FOLDER .. "\\update_temp.json")
                os.remove(UPDATES_FOLDER .. "\\items_version_temp.json")
            end)
        end)
    end)
end
...
function downloadUpdatedItems()
    ...
    lua_thread.create(function()
        ...
        downloadUrlToFile(download_url, UPDATES_FOLDER .. "\\items_new.json", function(id, status)
            if status == 1 then
                ...
                -- Копируем новый файл
                os.rename(UPDATES_FOLDER .. "\\items_new.json", ITEMS_FILE)
                ...
                sampAddChatMessage('[RodinaMarket] {ff0000}Ошибка загрузки обновления items.json', -1)
            end
        end)
    end)
end
...
function downloadScriptUpdate()
    ...
    lua_thread.create(function()
        ...
        downloadUrlToFile(download_url, new_script_path, function(id, status)
            if status == 1 then
                ...
                sampAddChatMessage('[RodinaMarket] {ff0000}Ошибка загрузки обновления скрипта', -1)
            end
        end)
    end)
end
