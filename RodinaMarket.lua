...
local GITHUB_RAW_URL = "https://raw.githubusercontent.com/RuRamzes/RodinaMarket/main/"
local UPDATE_CHECK_URL = GITHUB_RAW_URL .. "update.json"
local ITEMS_VERSION_URL = GITHUB_RAW_URL .. "items_version.json"
local ITEMS_DOWNLOAD_URL = GITHUB_RAW_URL .. "items.json"
...
local UPDATES_FOLDER = BASE_FOLDER .. "\\updates"

-- Создаем папки если их нет
local function createFolders()
    local folders = {BASE_FOLDER, DATA_FOLDER, ITEMS_FOLDER, LOGS_FOLDER, UPDATES_FOLDER}
...
local App = {
    ...
    update_info = nil,
    items_update_available = false,
    items_update_info = nil,
...
function loadItemNames()
    local items_file = getWorkingDirectory() .. "\\items.json"
...
function checkForUpdates()
    if App.checking_updates then return end
    App.checking_updates = true
    
    lua_thread.create(function()
        local time_salt = os.time()
        local temp_script_path = UPDATES_FOLDER .. "\\update_temp_" .. time_salt .. ".json"
        local temp_items_path = UPDATES_FOLDER .. "\\items_version_temp_" .. time_salt .. ".json"
        local update_url = UPDATE_CHECK_URL .. "?t=" .. time_salt
        
        downloadUrlToFile(update_url, temp_script_path, function(id, status, p1, p2)
            if status == dlstatus.STATUSEX_ENDDOWNLOAD then
                if doesFileExist(temp_script_path) then
                    local update_data = loadJSONFile(temp_script_path)
                    if update_data and update_data.version then
                        local current = parseVersion(SCRIPT_VERSION)
                        local latest = parseVersion(update_data.version)
                        
                        if compareVersions(latest, current) > 0 then
                            App.update_available = true
                            App.update_info = update_data
                            sampAddChatMessage(string.format('[RodinaMarket] {ffff00}Доступно обновление скрипта v%s -> v%s', 
                                SCRIPT_VERSION, update_data.version), -1)
                        end
                    end
                    
                    -- Проверяем обновление items.json ПОСЛЕ успешной проверки скрипта
                    local items_version_url = ITEMS_VERSION_URL .. "?t=" .. os.time()
                    downloadUrlToFile(items_version_url, temp_items_path, function(id2, status2, p3, p4)
                        if status2 == dlstatus.STATUSEX_ENDDOWNLOAD then
                            if doesFileExist(temp_items_path) then
                                local items_version_data = loadJSONFile(temp_items_path)
                                if items_version_data and items_version_data.version then
                                    local current = parseVersion(Data.settings.items_version)
                                    local latest = parseVersion(items_version_data.version)
                                    
                                    if compareVersions(latest, current) > 0 then
                                        App.items_update_available = true
                                        App.items_update_info = items_version_data
                                        sampAddChatMessage(string.format('[RodinaMarket] {ffff00}Доступно обновление items.json v%s -> v%s', 
                                            Data.settings.items_version, items_version_data.version), -1)
                                        
                                        -- Автоматически скачиваем новую версию items.json
                                        downloadUpdatedItems()
                                    end
                                end
                            end
                            -- Удаляем временные файлы
                            pcall(os.remove, temp_script_path)
                            pcall(os.remove, temp_items_path)
                            App.checking_updates = false
                            Data.settings.last_update_check = os.time()
...
function downloadUpdatedItems()
    if not App.items_update_info then return end
    
    lua_thread.create(function()
        local download_url = App.items_update_info.download_url or ITEMS_DOWNLOAD_URL
        download_url = download_url .. "?t=" .. os.time()
        local dest_path = UPDATES_FOLDER .. "\\items_new.json"

        -- Удаляем временный файл если он уже есть, чтобы не было ошибок записи
        if doesFileExist(dest_path) then os.remove(dest_path) end
        
        downloadUrlToFile(download_url, dest_path, function(id, status, p1, p2)
            if status == dlstatus.STATUS_DOWNLOADINGDATA then
                -- Можно вывести прогресс в консоль, если нужно
                -- print(string.format('Загружено %d из %d.', p1, p2))
            elseif status == dlstatus.STATUSEX_ENDDOWNLOAD then
                if doesFileExist(dest_path) then
                    -- Создаем резервную копию старого файла
                    if doesFileExist(ITEMS_FILE) then
                        os.rename(ITEMS_FILE, ITEMS_FOLDER .. "\\items_backup.json")
                    end
                    
                    -- Копируем новый файл
                    local rename_status = os.rename(dest_path, ITEMS_FILE)
                    
                    if rename_status then
                        -- Сохраняем новую версию
                        saveJSONFile(ITEMS_VERSION_FILE, {
                            version = App.items_update_info.version,
                            updated_at = os.time()
                        })
                        
                        Data.settings.items_version = App.items_update_info.version
                        saveJSONFile(CONFIG_PATHS.settings, Data.settings)
                        
                        -- Перезагружаем названия предметов
                        loadItemNames()
                        
                        App.items_update_available = false
                        App.items_update_info = nil
                        
                        sampAddChatMessage('[RodinaMarket] {00ff00}items.json успешно обновлен!', -1)
                    else
                        sampAddChatMessage('[RodinaMarket] {ff0000}Ошибка при замене файла items.json', -1)
                        -- Возвращаем бэкап если что-то пошло не так
                        if doesFileExist(ITEMS_FOLDER .. "\\items_backup.json") then
                            os.rename(ITEMS_FOLDER .. "\\items_backup.json", ITEMS_FILE)
                        end
                    end
                else
                    sampAddChatMessage('[RodinaMarket] {ff0000}Ошибка: файл items.json не был скачан (путь не найден)', -1)
                end
            end
        end)
    end)
...
function downloadScriptUpdate()
    if not App.update_info then return end
    
    lua_thread.create(function()
        local download_url = App.update_info.download_url
        download_url = download_url .. "?t=" .. os.time()
        
        local new_script_path = UPDATES_FOLDER .. "\\RodinaMarket_" .. App.update_info.version .. ".lua"
        if doesFileExist(new_script_path) then os.remove(new_script_path) end

        downloadUrlToFile(download_url, new_script_path, function(id, status, p1, p2)
            if status == dlstatus.STATUSEX_ENDDOWNLOAD then
                if doesFileExist(new_script_path) then
                    sampAddChatMessage(string.format('[RodinaMarket] {00ff00}Новая версия скрипта скачана: %s', new_script_path), -1)
                    sampAddChatMessage('[RodinaMarket] {ffff00}Перезапустите скрипт вручную для применения обновления.', -1)
                    App.update_available = false
                    App.update_info = nil
                else
                    sampAddChatMessage('[RodinaMarket] {ff0000}Ошибка загрузки обновления скрипта (файл не создан)', -1)
                end
            end
        end)
    end)
...
function parseVersion(version_str)
    local parts = {}
    for part in version_str:gmatch("%d+") do
...
imgui.OnFrame(function() return App.win_state[0] end, function(player)
    ...
    if imgui.Begin("Rodina Market", App.win_state, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize) then
        ...
        imgui.BeginChild("Sidebar", imgui.ImVec2(sidebar_width, 0), true)
            ...
            -- Добавляем кнопку обновлений в сайдбар
            if RenderSidebarItem(u8"Обновления", fa('arrows_rotate'), App.active_tab == 4, imgui.ImVec2(sidebar_width - 20, 45)) then
                App.active_tab = 4
            end
            ...
            imgui.SetCursorPos(imgui.ImVec2(20, win_h - 40))
            imgui.TextColored(imgui.ImVec4(0.4, 0.4, 0.4, 1.0), "v" .. SCRIPT_VERSION)
            
            -- Версия items.json
            imgui.SetCursorPos(imgui.ImVec2(20, win_h - 60))
            imgui.TextColored(imgui.ImVec4(0.4, 0.4, 0.4, 1.0), "items v" .. Data.settings.items_version)
...
function renderUpdatesTab()
    imgui.BeginChild("UpdatesContent", imgui.ImVec2(0, 0), true)
    
    imgui.TextColored(imgui.ImVec4(1, 1, 0, 1), u8("Система обновлений"))
    imgui.Separator()
    
    -- Статус обновлений
    imgui.Text(u8("Текущая версия скрипта: v" .. SCRIPT_VERSION))
    imgui.Text(u8("Текущая версия items.json: v" .. Data.settings.items_version))
    
    if Data.settings.last_update_check > 0 then
        imgui.Text(u8("Последняя проверка: " .. os.date("%Y-%m-%d %H:%M:%S", Data.settings.last_update_check)))
    end
    
    imgui.Separator()
    
    -- Кнопки управления
    if imgui.Button(u8("Проверить обновления"), imgui.ImVec2(S(200), S(30))) then
        checkForUpdates()
    end
    
    imgui.SameLine()
    
    if imgui.Button(u8("Принудительно обновить items.json"), imgui.ImVec2(S(250), S(30))) then
        App.items_update_available = true
        App.items_update_info = { version = "0.0.0" }
        downloadUpdatedItems()
    end
    ...
    if App.update_available then
        imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8("Доступно обновление скрипта!"))
        if App.update_info then
            imgui.Text(u8("Новая версия: v" .. App.update_info.version))
            if App.update_info.changelog then
                imgui.Text(u8("Изменения:"))
                for _, change in ipairs(App.update_info.changelog) do
                    imgui.Text(u8("  • " .. change))
                end
            end
        end
    else
        imgui.Text(u8("Обновлений скрипта не найдено"))
    end
    
    if App.items_update_available then
        imgui.TextColored(imgui.ImVec4(1, 0.5, 0, 1), u8("Доступно обновление items.json!"))
    else
        imgui.Text(u8("items.json актуален"))
    end
    
    imgui.Separator()
    
    -- Информация о файлах
    imgui.Text(u8("Пути к файлам:"))
    imgui.Text(u8("Папка данных: " .. DATA_FOLDER))
    imgui.Text(u8("Папка предметов: " .. ITEMS_FOLDER))
    imgui.Text(u8("Папка обновлений: " .. UPDATES_FOLDER))
...
function main()
    ...
    sampRegisterChatCommand('rupdate', function() checkForUpdates() end)
    
    -- Загружаем данные
    loadConfigData()
    loadItemNames()
    
    -- Проверяем обновления при запуске (раз в день)
    if os.time() - Data.settings.last_update_check > 86400 then
        checkForUpdates()
    end
    
    sampAddChatMessage('[RodinaMarket] {00ff00}Загружен! v' .. SCRIPT_VERSION, -1)
    sampAddChatMessage('[RodinaMarket] {00ff00}Папка данных: ' .. BASE_FOLDER, -1)
    sampAddChatMessage('[RodinaMarket] {ffff00}Введите {FF0000}/rmenu {FFFF00}для открытия меню', -1)
    sampAddChatMessage('[RodinaMarket] {ffff00}Введите {FF0000}/rupdate {FFFF00}для проверки обновлений', -1)
