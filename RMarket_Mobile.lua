...
local function asyncHttpRequest(method, url, args, resolve, reject)
    local thread_func = assert(loadstring([[
    ...
    local request_thread = effil.thread(thread_func)(method, url, args)
        ...
        while true do
            ...
            if not err then
                if status == 'completed' then
                    ...
                    if result then
                        if resolve then resolve(response) end
                    else
                        if reject then reject(response) end
                    end
                    return
...
local function checkUpdate()
    ...
    asyncHttpRequest("GET", url, {timeout = 5, verify = false}, function(res)
        if res.status_code == 200 then
            ...
            if json_ok and type(data) == "table" then
                ...
                sampAddChatMessage("{D9534F}[RMarket] {FFFFFF}Сбой: Неверный формат файла update.json.", -1)
            end
        end
    end, function(err)
...
local function downloadScriptUpdate(url, dest, callback)
    asyncHttpRequest("GET", url, {timeout = 15, verify = false, allow_redirects = true}, function(res)
        if res.status_code == 200 then
            ...
            if file then
                ...
                callback(false, "Нет прав на запись файла")
            end
        else
            callback(false, "Код ошибки HTTP: " .. tostring(res.status_code))
        end
    end, function(err)
        callback(false, "Сбой загрузки")
    end)
end
