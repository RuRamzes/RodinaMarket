...
local function checkUpdate()
    lua_thread.create(function()
        local ok, res = pcall(requests.get, "https://raw.githubusercontent.com/MTGMODS/rmarket/main/update.json", {timeout = 5, verify = false})
...
imgui.OnFrame(
    ...
    function(this)
        ...
        if show_update_window[0] then
            ...
            imgui.PushStyleColor(imgui.Col.Border, THEME.accent_primary)
            imgui.PushStyleVarFloat(imgui.StyleVar.WindowRounding, 16.0)
            imgui.PushStyleVarFloat(imgui.StyleVar.WindowBorderSize, 2.0)
            imgui.Begin("##UpdateModal", show_update_window, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.AlwaysAutoResize)
            
            imgui.PushFont(font_fa_large)
            imgui.TextColored(THEME.accent_primary, fa('circle_info'))
            imgui.PopFont()
            imgui.SameLine(0, 15)
            imgui.PushFont(font_main)
            imgui.TextColored(THEME.text_primary, u8"Доступно обновление RMarket Mobile!")
            imgui.Separator()
            
            imgui.TextColored(THEME.text_secondary, u8"Список изменений в версии " .. u8(State.update.version) .. ":")
            imgui.Dummy(imgui.ImVec2(0, 5))
            
            for line in State.update.text:gmatch("[^\r\n]+") do
                imgui.TextColored(THEME.text_primary, " •  " .. u8(line))
            end
            
            imgui.Dummy(imgui.ImVec2(0, 15))
            imgui.Separator()
            
            if imgui.Button(u8"Напомнить позже", imgui.ImVec2(250, 45)) then
                show_update_window[0] = false
            end
            imgui.SameLine()
            
            local btn_dl = imgui.GetWindowDrawList()
            local btn_p = imgui.GetCursorScreenPos()
            local clicked = false
            if imgui.InvisibleButton("##btn_update", imgui.ImVec2(250, 45)) then clicked = true end
            local btn_bg = imgui.IsItemActive() and imgui.ImVec4(THEME.accent_success.x*0.8, THEME.accent_success.y*0.8, THEME.accent_success.z*0.8, 1) or THEME.accent_success
            btn_dl:AddRectFilled(btn_p, imgui.ImVec2(btn_p.x + 250, btn_p.y + 45), imgui.ColorConvertFloat4ToU32(btn_bg), 12.0)
            
            local up_txt = u8"ОБНОВИТЬ СКРИПТ"
            local up_sz = imgui.CalcTextSize(up_txt)
            btn_dl:AddText(imgui.ImVec2(btn_p.x + (250 - up_sz.x)/2, btn_p.y + (45 - up_sz.y)/2), 0xFFFFFFFF, up_txt)
            
            if clicked then
                show_update_window[0] = false
                lua_thread.create(function()
                    sampAddChatMessage("{F0AD4E}[RMarket] {FFFFFF}Начинаю загрузку обновления...", -1)
                    downloadUrlToFile(State.update.url, thisScript().path, function(id, status, p1, p2)
                        if status == 6 then
                            sampAddChatMessage("{5CB85C}[RMarket] {FFFFFF}Обновление загружено! Скрипт перезапускается...", -1)
                            thisScript():reload()
                        end
                    end)
                end)
