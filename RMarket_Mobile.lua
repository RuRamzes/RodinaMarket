...
local function checkUpdate()
    lua_thread.create(function()
        local url = "https://raw.githubusercontent.com/RuRamzes/RodinaMarket/main/update.json"
        local ok, res = pcall(requests.get, url, {timeout = 5, verify = false})
        
        if not ok then
            sampAddChatMessage("{D9534F}[RMarket] {FFFFFF}Сбой проверки обновлений (библиотека requests).", -1)
            return
        end
        ...
        if res.status_code == 200 then
            ...
            if json_ok and type(data) == "table" then
                ...
                sampAddChatMessage("{D9534F}[RMarket] {FFFFFF}Сбой: Неверный формат файла update.json в облаке.", -1)
                print("RMarket Update Error: Invalid JSON. Text: " .. tostring(res.text))
            end
        elseif res.status_code ~= 404 then
            sampAddChatMessage("{D9534F}[RMarket] {FFFFFF}Ошибка сети при проверке обновлений (Код: " .. tostring(res.status_code) .. ")", -1)
        end
    end)
end

local function downloadScriptUpdate(url, dest)
    local ok, res = pcall(requests.get, url, {timeout = 15, verify = false})
    if not ok then return false, "Сбой библиотеки requests" end
    ...
    if res.status_code == 200 then
        ...
        return false, "Код ошибки HTTP: " .. tostring(res.status_code)
    end
end
            ...
            if imgui.Begin("##UpdateModal", show_update_window, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.AlwaysAutoResize) then
                ...
                if drawUpdBtn("##btn_upd_now", "ОБНОВИТЬ", fa('download'), p.x + 20 + btn_w + 20, btn_y, btn_w, THEME.accent_success) then
                    ...
                    lua_thread.create(function()
                        ...
                        local success, reason = downloadScriptUpdate(State.update.url, thisScript().path)
                        if success then
