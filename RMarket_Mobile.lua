...
local search_thread_1 = nil
local search_thread_4 = nil
local market_search_results_sell = {}
local market_search_results_buy = {}
...
local search_buf_market = ffi.new('char[128]')
...
local average_prices = loadJsonFile(PATH_PRICES)
    ...
    function(this)
        ...
        if imgui.Begin("RMarketMobile", win_state, flags) then
            ...
            if cfg_modal.active then
                ...
                dl:AddText(imgui.ImVec2(m_x + (m_w - n_sz.x)/2, m_y + 105), imgui.ColorConvertFloat4ToU32(THEME.accent_success), item_name)
                ...
                if average_prices then
                    ...
                    if average_prices[clean_k] then
                        local p = cfg_modal.is_sell and average_prices[clean_k].sell or average_prices[clean_k].buy
                        if p and p > 0 then avg_txt = formatMoney(p) .. " $" end
                    end
                end
                local avg_u8 = u8"Средняя цена: " .. avg_txt
                local a_sz = imgui.CalcTextSize(avg_u8)
                dl:AddText(imgui.ImVec2(m_x + (m_w - a_sz.x)/2, m_y + 130), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), avg_u8)

                local input_w = m_w - 70
                local input_x = m_x + 35
                
                local price_y = m_y + 170
                ...
                local amount_y = price_y + 115
                local amt_label = u8"Количество:"
                if not cfg_modal.is_sell then
                ...
                if cfg_modal.is_sell then
                    local max_btn_w = 90.0
                    local inp_w = input_w - max_btn_w - 15.0
                    
                    if cfg_modal.auto_max then
                        dl:AddRectFilled(imgui.ImVec2(input_x, amount_y + 35), imgui.ImVec2(input_x + inp_w, amount_y + 35 + 50), imgui.ColorConvertFloat4ToU32(THEME.bg_main), 12.0)
                        local txt = u8"Все доступные"
                        local sz = imgui.CalcTextSize(txt)
                        dl:AddText(imgui.ImVec2(input_x + (inp_w - sz.x)/2, amount_y + 35 + (50 - sz.y)/2), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), txt)
                    else
                    ...
                    imgui.SetCursorScreenPos(imgui.ImVec2(max_x, amount_y + 35))
                    if imgui.InvisibleButton("##btn_max", imgui.ImVec2(max_btn_w, 50.0)) then
                        cfg_modal.auto_max = not cfg_modal.auto_max
                    end
                    ...
                    dl:AddRectFilled(imgui.ImVec2(max_x, amount_y + 35), imgui.ImVec2(max_x + max_btn_w, amount_y + 35 + 50), imgui.ColorConvertFloat4ToU32(max_bg), 12.0)
                    
                    imgui.PushFont(font_main)
                    local max_txt = "MAX"
                    local max_sz = imgui.CalcTextSize(max_txt)
                    local txt_col = cfg_modal.auto_max and THEME.bg_main or THEME.text_primary
                    dl:AddText(imgui.ImVec2(max_x + (max_btn_w - max_sz.x)/2, amount_y + 35 + (50 - max_sz.y)/2), imgui.ColorConvertFloat4ToU32(txt_col), max_txt)
                    imgui.PopFont()
                else
                    imgui.SetNextItemWidth(input_w)
                    imgui.InputText("##cfg_amount", cfg_modal.buf_amount, 32, imgui.InputTextFlags.CharsDecimal)
                ...
                if DrawModalBtn("##m_save", "СОХРАНИТЬ", fa('check'), input_x, btn_y_1, input_w, THEME.accent_success) then
                    ...
                    if cfg_modal.is_sell then
                        cfg_modal.target_table[cfg_modal.index].auto_max = cfg_modal.auto_max
                    end
                ...
                if DrawModalBtn("##m_del", "УДАЛИТЬ", fa('trash'), input_x + sub_w + btn_gap, btn_y_2, sub_w, THEME.accent_danger) then
                    ...
                    table.insert(undo_stack, {
                        list = cfg_modal.is_sell and "sell" or "buy",
                        index = cfg_modal.index,
                        item = deleted_copy
                    })
            ...
            right_cursor = right_cursor - 10.0
            ...
            if active_tab == 4 then
                ...
                local filter_w = 180.0
                right_cursor = right_cursor - filter_w
                imgui.SetCursorScreenPos(imgui.ImVec2(right_cursor, btn_y))
                
                local current_srv_name = srv_names[StateMarket.server_filter_idx + 1]
                
                if DrawStyledButton("##srv_filter_hdr", u8(current_srv_name), filter_w, btn_sz, THEME.bg_tertiary, fa('server'), dl) then
                    imgui.OpenPopup("ServerFilterPopupHeaderMobile")
                end
                
                imgui.SetNextWindowPos(imgui.ImVec2(right_cursor, btn_y + btn_sz + 5))
            ...
            if active_tab == 1 or active_tab == 2 then
                right_cursor = right_cursor - btn_sz
                imgui.SetCursorScreenPos(imgui.ImVec2(right_cursor, btn_y))
                ...
                if active_tab == 1 then
                    ...
                    imgui.SetCursorScreenPos(imgui.ImVec2(right_cursor, btn_y))
                    if imgui.InvisibleButton("##dl_prices_btn", imgui.ImVec2(btn_sz, btn_sz)) then
                        api_DownloadAveragePrices()
                    end
                    local p_hov = imgui.IsItemHovered()
                    local p_col = imgui.ImVec4(0.2, 0.6, 0.9, 1.0)
                    if imgui.IsItemActive() then dl:AddCircleFilled(imgui.ImVec2(right_cursor + btn_sz/2, btn_y + btn_sz/2), btn_sz/2, imgui.ColorConvertFloat4ToU32(imgui.ImVec4(p_col.x, p_col.y, p_col.z, 0.3))) end
                    
                    imgui.PushFont(font_fa_large)
                    local px_sz = imgui.CalcTextSize(fa('cloud_arrow_down'))
                    dl:AddText(imgui.ImVec2(right_cursor + (btn_sz - px_sz.x)/2, btn_y + (btn_sz - px_sz.y)/2), imgui.ColorConvertFloat4ToU32(p_hov and THEME.text_primary or p_col), fa('cloud_arrow_down'))
                    imgui.PopFont()
                end
            end

            local search_w = win_w * 0.35
            local search_h = 45.0
            ...
            local function DrawMobileTab(id, title, icon, x_offset)
                ...
                if imgui.InvisibleButton("##tab_"..id, imgui.ImVec2(tab_w, tab_h)) then
                    active_tab = id
            ...
            DrawMobileTab(3, "ЛОГИ", fa('file_lines'), tab_w * 2)
            DrawMobileTab(4, "МАРКЕТ", fa('globe'), tab_w * 3)
            DrawMobileTab(5, "ПИАР", fa('bullhorn'), tab_w * 4)
            DrawMobileTab(6, "НАСТРОЙКИ", fa('gear'), tab_w * 5)
            ...
            if active_tab == 1 or active_tab == 2 then
                ...
                if imgui.BeginChild("SourceListCol", imgui.ImVec2(col_w, content_h - 10), false, imgui.WindowFlags.NoScrollbar) then
                    ...
                    if search_thread_1 and search_thread_1:status() ~= "dead" then
                        ...
                        CenterText(u8"Поиск...", THEME.accent_primary)
                        imgui.PopFont()
            ...
            elseif active_tab == 3 then
                ...
                local delta = DrawScrollZone("##scroll_logs", p.x + list_w + 20, content_y + 10, scroll_zone_w, content_h - 10)

                imgui.SetCursorScreenPos(imgui.ImVec2(p.x + 15, content_y + 10))
                imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0,0,0,0))
                if imgui.BeginChild("LogsChild", imgui.ImVec2(list_w, content_h - 10), false, imgui.WindowFlags.NoScrollbar) then
                    local c_pos = imgui.GetWindowPos()
                    local c_size = imgui.GetWindowSize()
                    dl:PushClipRect(c_pos, imgui.ImVec2(c_pos.x + c_size.x, c_pos.y + c_size.y), true)
                    ...
                    local stat_card_h = 75.0
                    
                    local function drawStatCard(x, y, label, val, color)
                        dl:AddRectFilled(imgui.ImVec2(x, y), imgui.ImVec2(x + stat_card_w, y + stat_card_h), imgui.ColorConvertFloat4ToU32(imgui.ImVec4(color.x, color.y, color.z, 0.4)), 12.0)
                        dl:AddRect(imgui.ImVec2(x, y), imgui.ImVec2(x + stat_card_w, y + stat_card_h), imgui.ColorConvertFloat4ToU32(color), 12.0, 15, 2.0)
                        dl:AddText(imgui.ImVec2(x + 15, y + 10), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), u8(label))
                        dl:AddText(imgui.ImVec2(x + 15, y + 38), imgui.ColorConvertFloat4ToU32(THEME.text_primary), formatMoney(val) .. " $")
                    end
                    ...
                    imgui.Dummy(imgui.ImVec2(0, stat_card_h + 10))
                    
                    local filters_y = c_pos.y + stat_card_h + 20
                    ...
                    if imgui.BeginCombo("##date_combo", LogsState.logs_dates_cache[LogsState.logs_current_date_idx] or u8"Даты") then
                    ...
                    if #LogsState.current_view_logs == 0 then
                        ...
                        CenterText(u8"Нет операций за эту дату", THEME.text_secondary)
                    else
                        for i, log in ipairs(LogsState.current_view_logs) do
                            ...
                            dl:AddRectFilled(item_p, imgui.ImVec2(item_p.x + list_w, item_p.y + item_h), imgui.ColorConvertFloat4ToU32(THEME.bg_secondary), 12.0)
                            dl:AddRectFilled(item_p, imgui.ImVec2(item_p.x + 6, item_p.y + item_h), imgui.ColorConvertFloat4ToU32(acc_col), 12.0, 1 + 8)
                            
                            imgui.PushFont(font_fa)
                            local ic = is_sale and fa('arrow_up_from_bracket') or fa('cart_shopping')
                            dl:AddText(imgui.ImVec2(item_p.x + 20, item_p.y + (item_h - 20)/2), imgui.ColorConvertFloat4ToU32(acc_col), ic)
                            imgui.PopFont()
                            
                            dl:PushClipRect(imgui.ImVec2(item_p.x + 55, item_p.y), imgui.ImVec2(item_p.x + list_w - 180, item_p.y + item_h), true)
                            dl:AddText(imgui.ImVec2(item_p.x + 55, item_p.y + 12), imgui.ColorConvertFloat4ToU32(THEME.text_primary), u8(log.item))
                            local meta_str = string.format("%s | %s", log.player or "Игрок", log.date:match("%d%d:%d%d:%d%d") or "")
                            dl:AddText(imgui.ImVec2(item_p.x + 55, item_p.y + 40), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), u8(meta_str))
                            dl:PopClipRect()
                            
                            local total_str = formatMoney(log.total) .. " $"
                            local t_sz = imgui.CalcTextSize(total_str)
                            dl:AddText(imgui.ImVec2(item_p.x + list_w - t_sz.x - 20, item_p.y + 12), imgui.ColorConvertFloat4ToU32(acc_col), total_str)
                            ...
                            dl:AddText(imgui.ImVec2(item_p.x + list_w - d_sz.x - 20, item_p.y + 40), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), u8(details))
                            
                            imgui.Dummy(imgui.ImVec2(0, item_h + 10))
                        end
                    end
                    imgui.PopFont()
                    dl:PopClipRect()
            ...
            elseif active_tab == 4 then
                local m_scroll_w = 65.0
                local m_list_w = win_w - m_scroll_w - 30.0
                local m_delta = DrawScrollZone("##scroll_market", p.x + m_list_w + 20, content_y + 10, m_scroll_w, content_h - 10)

                imgui.SetCursorScreenPos(imgui.ImVec2(p.x + 15, content_y + 10))
                imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0,0,0,0))
                if imgui.BeginChild("MarketChild", imgui.ImVec2(m_list_w, content_h - 10), false, imgui.WindowFlags.NoScrollbar) then
                    local c_pos = imgui.GetWindowPos()
                    local c_size = imgui.GetWindowSize()
                    dl:PushClipRect(c_pos, imgui.ImVec2(c_pos.x + c_size.x, c_pos.y + c_size.y), true)
                    ...
                    if StateMarket.is_loading then
                        imgui.Dummy(imgui.ImVec2(0, 50))
                        CenterText(u8"Загрузка лавок с сервера...", THEME.accent_primary)
                    elseif StateMarket.selected_shop then
                        local shop = StateMarket.selected_shop
                        imgui.PushFont(font_main)
                        
                        local top_bar_h = 50.0
                        if DrawStyledButton("##mkt_back", u8"Назад к списку", 200, top_bar_h, THEME.bg_tertiary, fa('arrow_left'), dl) then
                            StateMarket.selected_shop = nil
                        end
                        ...
                        if DrawStyledButton("##mkt_gps", gps_btn_txt, 250, top_bar_h, gps_btn_col, fa('location_dot'), dl) then
                        ...
                        imgui.TextColored(shop.vip and imgui.ImVec4(1.0, 0.84, 0.0, 1.0) or THEME.accent_primary, u8(shop.nickname .. " | " .. srv_name))
                        imgui.Dummy(imgui.ImVec2(0, 10))
                        
                        if not StateMarket.details_tab then StateMarket.details_tab = 1 end
                        local d_tab_w = m_list_w / 2
                        local d_tab_h = 45.0
                        ...
                        dl:AddRectFilled(d_p, imgui.ImVec2(d_p.x + d_tab_w, d_p.y + d_tab_h), imgui.ColorConvertFloat4ToU32(is_s_act and THEME.bg_tertiary or THEME.bg_secondary), 10.0, 1 + 2)
                        ...
                        if is_s_act then dl:AddRectFilled(imgui.ImVec2(d_p.x, d_p.y + d_tab_h - 4.0), imgui.ImVec2(d_p.x + d_tab_w, d_p.y + d_tab_h), imgui.ColorConvertFloat4ToU32(THEME.accent_success)) end
                        ...
                        dl:AddRectFilled(imgui.ImVec2(d_p.x + d_tab_w, d_p.y), imgui.ImVec2(d_p.x + m_list_w, d_p.y + d_tab_h), imgui.ColorConvertFloat4ToU32(is_b_act and THEME.bg_tertiary or THEME.bg_secondary), 10.0, 4 + 8)
                        ...
                        if is_b_act then dl:AddRectFilled(imgui.ImVec2(d_p.x + d_tab_w, d_p.y + d_tab_h - 4.0), imgui.ImVec2(d_p.x + m_list_w, d_p.y + d_tab_h), imgui.ColorConvertFloat4ToU32(THEME.accent_primary)) end

                        imgui.SetCursorScreenPos(imgui.ImVec2(d_p.x, d_p.y + d_tab_h + 15))
                        ...
                        if #current_list == 0 then
                            imgui.Dummy(imgui.ImVec2(0, 50))
                            local empty_txt = u8"В этой категории товаров нет."
                            local e_sz = imgui.CalcTextSize(empty_txt)
                            imgui.SetCursorPosX((m_list_w - e_sz.x) / 2)
                            imgui.TextColored(THEME.text_secondary, empty_txt)
                        else
                            local cols = 2
                            local spacing = 15.0
                            local card_w = (m_list_w - spacing * (cols - 1)) / cols
                            local card_h = 95.0
                            ...
                            for i, item in ipairs(current_list) do
                                ...
                                dl:AddRectFilled(imgui.ImVec2(cx, cy), imgui.ImVec2(cx + card_w, cy + card_h), imgui.ColorConvertFloat4ToU32(THEME.bg_secondary), 12.0)
                                dl:AddRectFilled(imgui.ImVec2(cx, cy), imgui.ImVec2(cx + 6, cy + card_h), imgui.ColorConvertFloat4ToU32(is_sell and THEME.accent_success or THEME.accent_primary), 12.0, 1 + 8)
                                
                                local icon_sz = 36.0
                                dl:AddCircleFilled(imgui.ImVec2(cx + 15 + icon_sz/2, cy + 15 + icon_sz/2), icon_sz/2, imgui.ColorConvertFloat4ToU32(THEME.bg_tertiary))
                                imgui.PushFont(font_fa)
                                local ic = is_sell and fa('box') or fa('basket_shopping')
                                local isz = imgui.CalcTextSize(ic)
                                dl:AddText(imgui.ImVec2(cx + 15 + (icon_sz-isz.x)/2, cy + 15 + (icon_sz-isz.y)/2), imgui.ColorConvertFloat4ToU32(is_sell and THEME.accent_success or THEME.accent_primary), ic)
                                imgui.PopFont()
                                
                                local text_x = cx + 15 + icon_sz + 10
                                dl:PushClipRect(imgui.ImVec2(text_x, cy), imgui.ImVec2(cx + card_w - 5, cy + card_h), true)
                                dl:AddText(imgui.ImVec2(text_x, cy + 15), imgui.ColorConvertFloat4ToU32(THEME.text_primary), u8(item.name))
                                dl:PopClipRect()
                                
                                local price_str = formatMoney(item.price) .. " $"
                                dl:AddText(imgui.ImVec2(text_x, cy + 38), imgui.ColorConvertFloat4ToU32(is_sell and THEME.accent_success or THEME.accent_primary), price_str)
                                dl:AddText(imgui.ImVec2(text_x, cy + 62), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), u8(item.amount .. " шт."))
                            end
                            imgui.Dummy(imgui.ImVec2(0, math.ceil(#current_list / cols) * (card_h + spacing) + 10))
                        end
                        imgui.PopFont()
                    else
                        imgui.Dummy(imgui.ImVec2(0, 10))
                        ...
                        if #StateMarket.shops_list == 0 then
                            imgui.Dummy(imgui.ImVec2(0, 50))
                            imgui.PushFont(font_main)
                            local txt = u8"Нет активных лавок. Нажмите 'ОБНОВИТЬ СПИСОК' внизу."
                            local t_sz = imgui.CalcTextSize(txt)
                            imgui.SetCursorPosX((m_list_w - t_sz.x) / 2)
                            imgui.TextColored(THEME.text_secondary, txt)
                            imgui.PopFont()
                        else
                            ...
                            if search_q ~= "" then
                                ...
                                if search_thread_4 and search_thread_4:status() ~= "dead" then
                                    imgui.Dummy(imgui.ImVec2(0, 50))
                                    CenterText(u8"Ищем товары...", THEME.accent_primary)
                                elseif #market_search_results_sell == 0 and #market_search_results_buy == 0 then
                                    imgui.Dummy(imgui.ImVec2(0, 50))
                                    local txt = u8"Товары не найдены."
                                    local t_sz = imgui.CalcTextSize(txt)
                                    imgui.SetCursorPosX((m_list_w - t_sz.x) / 2)
                                    imgui.TextColored(THEME.text_secondary, txt)
                                else
                                    ...
                                    local col_gap = 15.0
                                    local col_w_s = (m_list_w - col_gap) / 2
                                    local item_card_h = 85.0
                                    local spacing = 12.0
                                    local start_pos = imgui.GetCursorScreenPos()
                                    
                                    local s_txt = u8"В ПРОДАЖЕ"
                                    local ss_sz = imgui.CalcTextSize(s_txt)
                                    dl:AddText(imgui.ImVec2(start_pos.x + (col_w_s - ss_sz.x)/2, start_pos.y), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), s_txt)
                                    
                                    local b_txt = u8"В СКУПКЕ"
                                    local bb_sz = imgui.CalcTextSize(b_txt)
                                    dl:AddText(imgui.ImVec2(start_pos.x + col_w_s + col_gap + (col_w_s - bb_sz.x)/2, start_pos.y), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), b_txt)
                                    
                                    local list_start_y = start_pos.y + 30.0
                                    local max_items = math.max(#results_sell, #results_buy)
                                    
                                    local function drawSearchCard(res, idx, cx, cy, c_w)
                                        imgui.SetCursorScreenPos(imgui.ImVec2(cx, cy))
                                        if imgui.InvisibleButton("##mkt_item_"..tostring(res.is_sell).."_"..idx, imgui.ImVec2(c_w, item_card_h)) then
                                            StateMarket.selected_shop = res.shop
                                            search_buf_market[0] = 0 
                                            StateMarket.details_tab = res.is_sell and 1 or 2
                                        end
                                        local is_hov = imgui.IsItemHovered()
                                        local bg_c = is_hov and THEME.bg_tertiary or THEME.bg_secondary
                                        
                                        dl:AddRectFilled(imgui.ImVec2(cx, cy), imgui.ImVec2(cx + c_w, cy + item_card_h), imgui.ColorConvertFloat4ToU32(bg_c), 12.0)
                                        dl:AddRectFilled(imgui.ImVec2(cx, cy), imgui.ImVec2(cx + 6, cy + item_card_h), imgui.ColorConvertFloat4ToU32(res.is_sell and THEME.accent_success or THEME.accent_primary), 12.0, 1 + 8)
                                        if res.shop.vip then
                                            dl:AddRect(imgui.ImVec2(cx, cy), imgui.ImVec2(cx + c_w, cy + item_card_h), imgui.ColorConvertFloat4ToU32(imgui.ImVec4(1.0, 0.84, 0.0, 0.5)), 12.0, 15, 1.5)
                                        end
                                        
                                        local icon_sz = 36.0
                                        dl:AddCircleFilled(imgui.ImVec2(cx + 15 + icon_sz/2, cy + (item_card_h - icon_sz)/2 + icon_sz/2), icon_sz/2, imgui.ColorConvertFloat4ToU32(THEME.bg_main))
                                        
                                        imgui.PushFont(font_fa)
                                        local icon = res.is_sell and fa('box') or fa('basket_shopping')
                                        local isz = imgui.CalcTextSize(icon)
                                        dl:AddText(imgui.ImVec2(cx + 15 + (icon_sz - isz.x)/2, cy + (item_card_h - isz.y)/2), imgui.ColorConvertFloat4ToU32(res.is_sell and THEME.accent_success or THEME.accent_primary), icon)
                                        imgui.PopFont()
                                        
                                        local txt_x = cx + 15 + icon_sz + 12
                                        dl:PushClipRect(imgui.ImVec2(txt_x, cy), imgui.ImVec2(cx + c_w - 70, cy + item_card_h), true)
                                        dl:AddText(imgui.ImVec2(txt_x, cy + 10), imgui.ColorConvertFloat4ToU32(THEME.text_primary), u8(res.item.name))
                                        
                                        local shop_info = res.shop.nickname .. " | " .. getServerDisplayName(res.shop.serverId)
                                        local s_col = res.shop.vip and imgui.ImVec4(1.0, 0.84, 0.0, 1.0) or THEME.text_secondary
                                        dl:AddText(imgui.ImVec2(txt_x, cy + 32), imgui.ColorConvertFloat4ToU32(s_col), u8(shop_info))
                                        
                                        local amt_str = "В наличии: " .. res.item.amount
                                        dl:AddText(imgui.ImVec2(txt_x, cy + 54), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), u8(amt_str))
                                        dl:PopClipRect()
                                        
                                        local price_str = formatMoney(res.item.price) .. " $"
                                        local psz = imgui.CalcTextSize(price_str)
                                        dl:AddText(imgui.ImVec2(cx + c_w - psz.x - 15, cy + (item_card_h - psz.y)/2), imgui.ColorConvertFloat4ToU32(res.is_sell and THEME.accent_success or THEME.accent_primary), price_str)
                                    end

                                    for i = 1, max_items do
                                        local cy = list_start_y + (i - 1) * (item_card_h + spacing)
                                        if i <= #results_sell then drawSearchCard(results_sell[i], i, start_pos.x, cy, col_w_s) end
                                        if i <= #results_buy then drawSearchCard(results_buy[i], i, start_pos.x + col_w_s + col_gap, cy, col_w_s) end
                                    end
                                    imgui.Dummy(imgui.ImVec2(0, 30.0 + max_items * (item_card_h + spacing) + 10))
                                end
                            else
                                ...
                                if #filtered_shops == 0 then
                                    imgui.Dummy(imgui.ImVec2(0, 50))
                                    local txt = u8"На этом сервере нет лавок."
                                    local t_sz = imgui.CalcTextSize(txt)
                                    imgui.SetCursorPosX((m_list_w - t_sz.x) / 2)
                                    imgui.TextColored(THEME.text_secondary, txt)
                                else
                                    ...
                                    for i, shop in ipairs(filtered_shops) do
                                        ...
                                        imgui.SetCursorScreenPos(imgui.ImVec2(cx, cy))
                                        if imgui.InvisibleButton("##shop_grid_"..i, imgui.ImVec2(card_w, card_h)) then
                                            StateMarket.selected_shop = shop
                                        end
                                        ...
                                        dl:AddRectFilled(imgui.ImVec2(cx, cy), imgui.ImVec2(cx + card_w, cy + card_h), imgui.ColorConvertFloat4ToU32(bg_col), 16.0)
                                        dl:AddRect(imgui.ImVec2(cx, cy), imgui.ImVec2(cx + card_w, cy + card_h), imgui.ColorConvertFloat4ToU32(border_col), 16.0, 15, shop.vip and 2.0 or 1.0)
                                        
                                        imgui.PushFont(font_fa_large)
                                        local icon_col = shop.vip and imgui.ImVec4(1.0, 0.84, 0.0, 1.0) or THEME.accent_primary
                                        dl:AddText(imgui.ImVec2(cx + 15, cy + 25), imgui.ColorConvertFloat4ToU32(icon_col), shop.vip and fa('crown') or fa('store'))
                                        imgui.PopFont()
                                        
                                        dl:AddText(imgui.ImVec2(cx + 65, cy + 15), imgui.ColorConvertFloat4ToU32(THEME.text_primary), u8(shop.nickname))
                                        
                                        local srv_name = getServerDisplayName(shop.serverId)
                                        dl:AddText(imgui.ImVec2(cx + 65, cy + 40), imgui.ColorConvertFloat4ToU32(THEME.text_secondary), u8(srv_name))
                                        
                                        local stats = string.format("Sell: %d | Buy: %d", shop.sell_count, shop.buy_count)
                                        dl:AddText(imgui.ImVec2(cx + 65, cy + 65), imgui.ColorConvertFloat4ToU32(imgui.ImVec4(0.4, 0.42, 0.5, 1.0)), u8(stats))
                                    end
                                    imgui.Dummy(imgui.ImVec2(0, total_rows * (card_h + spacing) + 10))
                                end
                            end
                            imgui.PopFont()
                        end
            ...
            elseif active_tab == 5 or active_tab == 6 then
                ...
                if active_tab == 5 then
                    if imgui.BeginChild("AutoPRChild", imgui.ImVec2(list_w, content_h - 10), false, imgui.WindowFlags.NoScrollbar) then
                        ...
                        for i, item in ipairs(Settings.auto_pr.items) do
                            ...
                            if imgui.InvisibleButton("##pr_act_"..i, imgui.ImVec2(50, 40)) then
                                item.active = not item.active
                                if item.active and Settings.auto_pr.active then item.next_send = os.time() + 2 end
                                saveJsonFile(PATH_SETTINGS, Settings)
                            end
                            dl:AddRectFilled(t_p, imgui.ImVec2(t_p.x + 50, t_p.y + 40), imgui.ColorConvertFloat4ToU32(item.active and THEME.accent_success or THEME.bg_main), 12.0)
                            if item.active then
                                imgui.PushFont(font_fa)
                                local check_sz = imgui.CalcTextSize(fa('check'))
                                dl:AddText(imgui.ImVec2(t_p.x + (50 - check_sz.x)/2, t_p.y + (40 - check_sz.y)/2), 0xFFFFFFFF, fa('check'))
                                imgui.PopFont()
                            end
                            ...
                            local ch_w = 90
                            local ch_p = imgui.ImVec2(t_p.x + 50 + 15, p_item.y + 15)
                            imgui.SetCursorScreenPos(ch_p)
                            if imgui.InvisibleButton("##pr_ch_"..i, imgui.ImVec2(ch_w, 40)) then
                                item.channel = (item.channel % 6) + 1
                                saveJsonFile(PATH_SETTINGS, Settings)
                            end
                            dl:AddRect(ch_p, imgui.ImVec2(ch_p.x + ch_w, ch_p.y + 40), ch_colors[curr_ch], 12.0, 15, 2.0)
                            local ch_txt = ch_names[curr_ch]
                            local ctx_sz = imgui.CalcTextSize(ch_txt)
                            dl:AddText(imgui.ImVec2(ch_p.x + (ch_w - ctx_sz.x)/2, ch_p.y + (40 - ctx_sz.y)/2), ch_colors[curr_ch], ch_txt)
                            
                            local del_w = 80
                            local del_p = imgui.ImVec2(ch_p.x + ch_w + 15, p_item.y + 15)
                            ...
                            local trash_sz = 40.0
                            local del_btn_p = imgui.ImVec2(p_item.x + list_w - trash_sz - 15, p_item.y + 15)
                            ...
                            dl:AddRectFilled(del_btn_p, imgui.ImVec2(del_btn_p.x + trash_sz, del_btn_p.y + trash_sz), imgui.ColorConvertFloat4ToU32(is_trash_hov and THEME.bg_main or THEME.bg_tertiary), 12.0)
                            imgui.PushFont(font_fa)
                            ...
                            imgui.SetCursorScreenPos(imgui.ImVec2(p_item.x + 15, p_item.y + 70))
                ...
                elseif active_tab == 6 then
                    if imgui.BeginChild("SettingsChild", imgui.ImVec2(list_w, content_h - 10), false, imgui.WindowFlags.NoScrollbar) then
                        ...
                        if imgui.InvisibleButton("##cb_show_hud", imgui.ImVec2(55, 55)) then
                            b_show_hud[0] = not b_show_hud[0]
                        end
                        local hud_hov = imgui.IsItemHovered()
                        local hud_bg = b_show_hud[0] and THEME.accent_success or (hud_hov and imgui.ImVec4(1,1,1,0.1) or THEME.bg_tertiary)
                        dl:AddRectFilled(hud_p, imgui.ImVec2(hud_p.x + 55, hud_p.y + 55), imgui.ColorConvertFloat4ToU32(hud_bg), 12.0)
                        if b_show_hud[0] then
                            imgui.PushFont(font_fa_large)
                            local isz = imgui.CalcTextSize(fa('check'))
                            dl:AddText(imgui.ImVec2(hud_p.x + (55 - isz.x)/2, hud_p.y + (55 - isz.y)/2), 0xFFFFFFFF, fa('check'))
                            imgui.PopFont()
                        end
            ...
            local btn_color = base_btn_col
            if btn_active and base_btn_col then
                btn_color = imgui.ImVec4(base_btn_col.x*0.8, base_btn_col.y*0.8, base_btn_col.z*0.8, 1.0)
            end
            ...
            elseif active_tab == 2 then
                ...
                action_icon = fa('shop')
            elseif active_tab == 3 then
                action_txt = u8"ОБНОВИТЬ ИСТОРИЮ"
                action_icon = fa('arrows_rotate')
            elseif active_tab == 4 then
                action_txt = u8"ОБНОВИТЬ СПИСОК ЛАВОК"
                action_icon = fa('arrows_rotate')
            elseif active_tab == 5 then
